...
int shit2;
int Cheat::CheatEntry(int argc, char** argv)
{
...
#pragma endregion
	...
	if (s_uid != "0000001") // if this is true then they tried launching the app directly or the id is wrong 
	{
	...
	hWindow = FindWindow(0, _T("Counter-Strike: Global Offensive"));
	...
	{
		Beep(550, 100);
		hWindow = FindWindow(NULL, _T("Counter-Strike: Global Offensive"));
		Sleep(100);
	} 
	Beep(750, 300);
...
void Cheat::Update()
{
	...
	{
		ReadProcessMemory(Game::hProcess, (void*)LocalPlayerAddress, &DWLocalPlayer, sizeof(DWLocalPlayer), 0);
		lPlayer.SetAddress(DWLocalPlayer);
		lPlayer.Update(1);
		...
		ReadProcessMemory(Game::hProcess, (void*)EntitiesAddress, &buffer, (sizeof(DWORD)* (64)), &bufferSize);
		for (int i = 0; i < 64; i++)
		{
	...
	if (lPlayer.m_TriggerToggle == true || lPlayer.IsKeyDown(0xDC)) // Backslash
	{
		lPlayer.Update(1);
